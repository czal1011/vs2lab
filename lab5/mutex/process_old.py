    ...
    def __receive(self):
        ...
        if _receive:
            ...
            self.logger.debug("{} received {} from {}.".format(
                ...
                else "HEARTBEAT_REQ" if msg[2] == HEARTBEAT_REQ
                else "HEARTBEAT_RES", self.__mapid(msg[1])))
            ...
            # TODO if(self.queue[0][nodeID] == crashedNodeID):
            # idea: multicast to all nodes in other_processes
            # collect responses until len(other_processes) - 2 is reached
            # remove the remaining process from other_processes and all_processes
            # downside: double waiting time :(
